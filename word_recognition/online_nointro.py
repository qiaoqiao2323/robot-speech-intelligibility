...
f.close()
PORT = 56680  # port to conect the robot
...
class vocabulary_TestApp(App):
    ...
    def playwav(self):
        self.sound = SoundLoader.load('sounds/1.wav')
        # 1, 3, 5, 7, 9.75, 10
    ...
    def build(self):
        ...
        self.volume_default =1.05
        self.speed_default = 77
        self.pitchShift_default = 95
        self.enunciation_default = 0.25
        # self.volume_default = 0.90
        # self.speed_default = 79
        # self.pitchShift_default = 102
        # self.enunciation_default = 0.26
        # [[1.0553246  77.076       0.9538781   0.25146016]
        #  [0.8962687  79.05078     1.0231192   0.26225844]]
        ...
        self.livre = Image(source="images/points.png", size_hint=(None, None), size=(20, 20),
                           pos_hint={'center_x': 0.92, 'center_y': 0.4})
        game.add_widget(self.livre)  # the pound symbol

        self.pleasant = Label(text="[color=#000004]Please rate your pleasant with my voice from 1 to 10[/color]", font_size='20sp',
    ...
    def speak(self, sentence):  # nao speech with no blockage of the rest of the application
        ...
        # tts.say("\\vct=self.pitchShift_default\\Hello my friends")
    ...
    def buttonClicked(self, btn):
        ...
        if (self.btn1.text == "start"):
            ...
            if (exists(fileID)):
                ...
                #                  The robot will randomly select a word and pronounce each letter one at a time.\\pau=1000\\
                ...
                #                  you have 50 words in total. \\pau=500\\
                ...
                # Clock.schedule_once(partial(self.part_3), 15)
                # Clock.schedule_once(partial(self.part_4), 25)
                # Clock.schedule_once(partial(self.part_5), 38)
                # Clock.schedule_once(partial(self.part_6), 49)
                # Clock.schedule_once(partial(self.part_7), 56)
                Clock.schedule_once(partial(self.opac), 1)  # reapper the 'start game' butto
