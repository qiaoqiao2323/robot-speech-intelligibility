...
# IP = "192.168.0.130"
f.close()
PORT = 55868  # port to conect the robot
...
class vocabulary_TestApp(App):
    ...
    def build(self):
        ...
        #
        self.volume_default = 1.3
        self.speed_default = 80
        self.pitchShift_default = 100
        self.enunciation_default = 0
        #
        # self.volume_default = 1.13
        # self.speed_default = 66
        # self.pitchShift_default = 90
        # self.enunciation_default = 0.33
        # self.volume_default = 1.1
        # self.speed_default = 68
        # self.pitchShift_default = 88
        # self.enunciation_default = 0.4
        # [1.3091557  79.88377     1.0855808   0.28451678] 5
        # [1.3084579  78.90372     1.0873663   0.265589]  4
        # [1.1309781  66.49858     0.90037733  0.3454099]  3
        # [1.1519074  67.78701     0.90762335  0.35585472]] 4
    ...
    def play(self, btn):
        ...
        proc_ph = subprocess.Popen([r"C:\Users\Administrator\PycharmProjects\openai\venv\Scripts\python.exe",
                                       r"C:\Users\Administrator\PycharmProjects\openai\main.py"],
                                      stdout=subprocess.PIPE, stdin=subprocess.PIPE)

        output_ph, error = proc_ph.communicate()


        english_words = self.load_words()

        self.random_word = output_ph
        speech_text = "\\vct={0}\\{1}".format(self.pitchShift_default, self.random_word)
    ...
    def change_value(self, btn):  # fonction called by the 'collect reward' button
        ...
        if (self.reward.opacity != 0):
            ...
            if (self.nb_bal >= 4):  # if  30 balloons have been used
                ...
                self.user_experience.text = self.remove_punctuation_and_letters(self.user_experience.text)
                self.files.write("%d , %.2f, %s , %s , %d ,%d , %d , %.2f , %.2f,%.2f , %.2f,%.2f \n" % (
                    self.nb_bal + 1, self.level, self.random_word, self.inputword, self.nb_letters, len(self.random_word),self.c,
                    self.volume_default, self.speed_default, self.pitchShift_default, self.enunciation_default, float(self.user_experience.text)))
                ...
                self.volume_default, self.speed_default, self.pitchShift_default, self.enunciation_default, float(self.user_experience.text)))
                self.nb_bal += 1
                self.tested.text = 'Phrases tested:' + str(self.nb_bal)
                self.nb_letters = 0
                self.txt2.text = ''
    ...
    def buttonClicked(self, btn):
        ...
        if (self.btn1.text == "start"):
            ...
            if (exists(fileID)):
                ...
                Clock.schedule_once(partial(self.part_2), 0.8)  # post rules paragraph per paragraph
                Clock.schedule_once(partial(self.part_3), 1.4)
                Clock.schedule_once(partial(self.part_4), 2.3)
                Clock.schedule_once(partial(self.part_5), 3.6)
                Clock.schedule_once(partial(self.part_6), 4.7)
                Clock.schedule_once(partial(self.part_7), 5.6)
                Clock.schedule_once(partial(self.opac), 6.1)  # reapper the 'start game' butto
